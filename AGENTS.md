...
- `npm run check:browser` - real-Chrome smoke test incl. true offline reload (needs local Chrome/Edge)
...
- **`sw.js` precache must cover every file in `assets/`** plus the core documents. Add an asset -> update `SHELL` and bump `CACHE_VERSION`. Enforced by tests + `check:links`. On localhost the SW runs network-first for everything (dev edits never go stale) while still populating the cache - so `check:browser` can do a genuine offline reload.
- **Strict CSP meta in `index.html`** (`script-src 'self'`). No inline `<script>` or inline event handlers - everything must live in `assets/` modules. `<style>`/inline `style=""` are allowed via `style-src 'unsafe-inline'`.
- **`commands.run(raw, {syncHash:false})`** skips address-bar sync - use it for non-user invocations (boot greeting) so a `#hash` deep link isn't clobbered before it's read.
